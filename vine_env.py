...
class Drone:
    ...
    def __init__(self, position: Tuple[float, float]):
        ...
        self.busy = False
        self.time_left = 0.0
        self.target_handover: Optional[int] = None
        self.delivered_count = 0
        self.battery = 100.0
...
class VineEnv(MultiAgentEnv):
    ...
    def __init__(
        ...
        # fatigue tuning (all are PER-SECOND rates)
        self.human_harvest_fatigue_rate = float(human_harvest_fatigue_rate)
    ...
    def step(self, action_dict: Dict[str, int]) -> Tuple[Dict, Dict, Dict, Dict, Dict]:
        ...
        for i, h in enumerate(self.humans):
            if h.busy:
                ...
                if h.current_action == ACTION_HARVEST:
                    h.fatigue = float(
                        np.clip(
                            h.fatigue + self.human_harvest_fatigue_rate * self.dt,
                            0.0, 1.0
                        )
                    )
                elif h.current_action in (ACTION_TRANSPORT, ACTION_ENQUEUE):
                    mult = getattr(h, "transport_fatigue_multiplier", 1.0)
                    fatigue_rate = self.human_transport_fatigue_rate * mult
                    h.fatigue = float(
                        np.clip(
                            h.fatigue + fatigue_rate * self.dt,
                            0.0, 1.0
                        )
                    )
                    ...
                    h.fatigue = float(
                        np.clip(
                            h.fatigue - self.human_rest_recovery_rate * self.dt,
                            0.0, 1.0
                        )
                    )
                if h.time_left <= 0.0:
                    ...
                    elif h.current_action == ACTION_ENQUEUE:
                        if h.has_box:
                            hp = self._get_line_handover(line)
                            hp["queued_boxes"] += 1
                            hp["queue_contributors"].append(i)
        ...
        for agent_id, a in action_dict.items():
            ...
            can_enqueue = (h.has_box or can_take_ready_box) and (hp["queued_boxes"] < self.max_backlog)
            ...
            if a == ACTION_HARVEST:
                ...
                if can_enqueue:
                    # If the worker is not already carrying, take one ready box from the line first
                    if not h.has_box:
        ...
        for d in self.drones:
            if d.busy:
                ...
                if d.status in (DRONE_GO_TO_HANDOVER, DRONE_DELIVER, DRONE_GO_TO_CHARGE):
                    drain_rate = self._get_drone_drain_rate(d)
                    d.battery = max(0.0, d.battery - drain_rate * self.dt)
                if d.time_left <= 0.0:
                    d.busy = False
                    d.time_left = 0.0
                    
                    if d.status == DRONE_GO_TO_HANDOVER:
                        hp = self.handover_points[d.target_handover]
                        d.position = hp["position"].copy()

                        if hp["queued_boxes"] > 0:
                            hp["queued_boxes"] -= 1
                            contributor = hp["queue_contributors"].popleft() if len(hp["queue_contributors"]) > 0 else None
        ...
        for d in self.drones:
            if (not d.busy) and d.status == DRONE_IDLE:
                ...
                candidates = [hp["id"] for hp in self.handover_points if hp["queued_boxes"] > 0]
                if candidates:
                    dists = [np.linalg.norm(self.handover_points[hid]["position"] - d.position) for hid in candidates]
                    target = candidates[int(np.argmin(dists))]

                    dist_to_handover = float(np.linalg.norm(self.handover_points[target]["position"] - d.position))
                    t_go = dist_to_handover / max(self.drone_speed, 1e-6)

                    dist_to_cp = float(np.linalg.norm(self.handover_points[target]["position"] - self.collection_point))
                    t_deliver = dist_to_cp / max(self.drone_speed, 1e-6)

                    battery_needed = (
                        self.drone_batt_drain_rate_unloaded * t_go
                        + self.drone_batt_drain_rate_loaded * t_deliver
                    )
                    safety_margin = 5.0
        ...
        team_reward = r_delivery + r_fatigue + r_backlog
        ...
        remaining_queue = int(sum(hp["queued_boxes"] for hp in self.handover_points))

        all_harvested = all((v.kg_remaining <= 1e-9) and (v.boxes_ready == 0) for v in self.vines)
        no_queue = all(hp["queued_boxes"] == 0 for hp in self.handover_points)
    ...
    def _get_drone_xyz(self, drone: Drone) -> np.ndarray:
        ...
        if np.allclose(drone.position, self.collection_point):
            ...
            z = self.charging_z
        elif drone.target_handover is not None:
            z = self.handover_points[drone.target_handover]["z"]
        else:
            # fallback: nearest handover altitude if available, else nearest line altitude
            if len(self.handover_points) > 0:
                dists = [np.linalg.norm(hp["position"] - drone.position) for hp in self.handover_points]
                nearest = int(np.argmin(dists))
                z = self.handover_points[nearest]["z"]
            else:
    ...
    def _get_drone_drain_rate(self, drone: Drone) -> float:
        """
        Battery drain rate in % per second.
        Loaded flight drains faster than unloaded flight.
        """
        if drone.status == DRONE_DELIVER or drone.has_box:
            return self.drone_batt_drain_rate_loaded
        return self.drone_batt_drain_rate_unloaded
    ...
    def _get_obs_for_agent(self, agent_idx: int) -> np.ndarray:
        ...
        # local service units = ready on line + queue at assigned handover point
        for slot in range(self.local_vine_k):
            if slot < len(local_indices):
                idx = int(local_indices[slot])
                v = self.vines[idx]
                hp = self.handover_points[v.handover_id]
                service_units = v.boxes_ready + hp["queued_boxes"]
        ...
        # IMPORTANT:
        # In synchronous mode, busy humans still receive observations every step.
        # While busy, only allow their current ongoing action in the mask.
        if h.busy:
            mask[:] = 0.0
            mask[int(h.current_action)] = 1.0
        else:
            can_harvest = (not h.has_box) and (line.kg_remaining > 1e-9)
            can_take_ready_box = (not h.has_box) and (line.boxes_ready > 0)
            ...
            if (h.has_box or can_take_ready_box) and hp["queued_boxes"] >= self.max_backlog:
                mask[ACTION_ENQUEUE] = 0.0
    ...
    def _render_pygame(self):
        ...
        for v in self.vines:
            ...
            if v.kg_remaining <= 1e-9 and v.boxes_ready == 0:
                color = (120, 120, 120)
            else:
                color = (34, 139, 34)
            ...
            text_str = f"{v.kg_remaining:.1f}kg | r{v.boxes_ready} | hp{v.handover_id}"
            text_surf = font.render(text_str, True, TEXT_COLOR)
            self._screen.blit(text_surf, (x + 12, y - 10))
        # Handover points (orange)
        for hp in self.handover_points:
            x, y = world_to_screen(hp["position"])
            pygame.draw.circle(self._screen, (255, 140, 0), (x, y), 7)
            hp_text = font.render(f"HP{hp['id']} q{hp['queued_boxes']}", True, TEXT_COLOR)
            self._screen.blit(hp_text, (x + 10, y + 2))
        ...
        # Drone stats
        for i, d in enumerate(self.drones):
            status_name = ["IDLE","GO_TO_HANDOVER","DELIVER", "GO_TO_CHRG", "CHARGING"][d.status]
